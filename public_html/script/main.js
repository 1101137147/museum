...
function initPage() {
	...
	for (i = 0; i < scratchers.length; i++) {
		...
		scratchers[i].addEventListener('imagesloaded', onScratcherLoaded);
                 localStorage.setItem('折扣', 50);
		scratchers[i].setImages('image/scra' + i1 + '-bg.jpg',
			'image/scra' + i1 + '-fg.jpg');
	}

	// get notifications of this scratcher changing
	// (These aren't "real" event listeners; they're implemented on top
	// of Scratcher.)
	scratchers[0].addEventListener('reset', scratcher3Changed);
	scratchers[0].addEventListener('scratch', scratcher3Changed);
